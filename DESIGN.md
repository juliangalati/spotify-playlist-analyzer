...
> **Note**: This design system is shared with the sibling `spotify-track-analyzer` project and copied here so this project is self-contained. If you change tokens here, change them there too (or vice versa) to keep the two apps visually coherent. Playlist-analyzer-specific additions live at the bottom.
...
## 10. Playlist-Analyzer Additions

These are tokens and components introduced by this project on top of the shared system above. Add any new additions here (don't sprinkle them throughout the sections above — the rest of this file mirrors the sibling project).

### Tokens

```css
:root {
  /* hover bg for track cards in the virtualized gallery */
  --row-hover: #1f1f1f;

  /* coverage badge */
  --coverage-border: #7c7c7c;
  --coverage-text:   #b3b3b3;
}
```

### Components

**Coverage badge** — outlined pill shown on the playlist header when `analyzed < total`.
- Background: transparent
- Border: 1px solid `#7c7c7c`
- Text: `#b3b3b3`, 12px weight 700, uppercase, letter-spacing 1.4px
- Padding: 4px 12px
- Radius: 9999px

**Sort pills** — small pill buttons above the track gallery.
- Inactive: transparent bg, `#7c7c7c` border, `#b3b3b3` text
- Active: transparent bg, `#1ed760` border, `#1ed760` text
- Hover (inactive): border becomes `#ffffff`
- Padding: 6px 14px
- Font: 12px weight 700 uppercase, 1.4px letter-spacing
- Active pill also shows an arrow (`↑` / `↓`) reflecting sort direction

**Virtualized scroll container**
- `overflow-y: auto`
- `max-height: 70vh` (gives the virtualizer a bounded scroll parent)
- Background: inherits page `#121212`
- Inner content div takes `height: totalRowsCount * rowHeight` so the scrollbar reflects the full data set

**Track card (in gallery)**
- Background: `--surface` (`#181818`)
- Padding: 12px
- Radius: 8px
- Hover: bg lifts to `--row-hover`, shadow-medium
- Cover: 100% width, `aspect-ratio: 1 / 1`, 4px radius
- Missing-features state: stats area rendered in `--text-muted` with `—` placeholders; tooltip on hover via `title` attribute

**Camelot distribution slot** — see `CAMELOT.md` for the exact opacity scale. The "most-populated" stroke is `var(--accent)` at 2px; all other strokes are `var(--bg)` at 2px for clean segment separation.

### Don'ts specific to this app

- Don't render Spotify Green in the playlist header unless a value needs emphasis (the sibling app uses it only for the Camelot card). The header should stay achromatic so the cover art is the color anchor.
- Don't use the green accent stroke on more than one wheel slot at a time. If there's a tie for most-populated slot, prefer the lowest-numbered slot.
- Don't let the track gallery grow unbounded. `max-height: 70vh` is mandatory for virtualization to work correctly.
